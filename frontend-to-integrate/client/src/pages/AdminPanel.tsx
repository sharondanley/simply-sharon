...
const API = {
  /** Sign in. Should return the authenticated user or throw on bad credentials. */
  ...
  async getCurrentUser(): Promise<AuthUser | null> {
    return null; // e.g. const r = await fetch("/api/auth/me"); return r.ok ? r.json() : null;
  },

  /** Return all posts (published + drafts) for the admin list. */
  async listPosts(_page: number, _limit: number): Promise<{ items: PostItem[]; total: number }> {
    return { items: [], total: 0 };
  },

  /** Create a new post. */
  async createPost(_data: CreatePostInput): Promise<{ slug: string }> {
    throw new Error("API.createPost not implemented");
  },

  /** Hard-delete a post. */
  async deletePost(_id: number): Promise<void> {
    throw new Error("API.deletePost not implemented");
  },
  ...
  async uploadThumbnail(
    ...
    throw new Error("API.uploadThumbnail not implemented");
  },
};
